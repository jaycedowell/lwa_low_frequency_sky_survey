...
wsclean -size 350 350 -scale 20amin -niter 75000 -threshold ${threshold} -weight natural -multiscale -multiscale-scale-bias 0.9 -mgain 0.6 -nonegative -join-polarizations -pol xx,yy -channel-range 700 740 -no-update-model-required -fitsmask ${SCRIPT_PATH}/mask_350.fits -name ${outname} ${shmname} > ${outname}.log 2>&1
