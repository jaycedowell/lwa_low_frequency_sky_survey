...
from lsl.writer import measurementset as fitsidi
...
def processChunk(idf, site, good, filename, LFFT=64, Overlap=1, pols=['xx','yy']):
    ...
    for pol in pols:
        ...
        for k in xrange(nSec):
            ...
            toUse = numpy.where( (freq>=12.0e6) & (freq<=86.0e6) )
            toUse = toUse[0]
        ...
        if pol == pols[0]:
            ...
            fits = fitsidi.MS(filename, refTime=refTime)
            fits.setStokes(pols)
...
def main(args):
    ...
    for i in xrange(len(antennas)):
        ant = antennas[i]
        if ant.getStatus() < 22 and not config['all']:
            pass
        elif ant.stand.id in (35, 256, 257, 258, 259):
            pass
        else:
            if ant.pol == 0:
                goodX.append(ant)
